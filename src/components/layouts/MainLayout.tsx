...
// @ts-ignore
import logo from "~/assets/imgs/icebear.jpeg";
// @ts-ignore
import near from "~/assets/imgs/brand-icon.png";
import { Discord, Telegram, Twitter, Medium } from "~components/icons/Social";

import { DownOutlined, UserOutlined, LogoutOutlined, SolutionOutlined, MenuOutlined, DollarOutlined } from '@ant-design/icons';
...
const MainLayout = (props: any) => {
    ...
    return (
      <div>
          <Layout className={"bg-color sm:px-0 md:px-3 xl:px-5 lg:px-5"} style={{minHeight: "100vh"}}>
              <Header className={"padding-0-15 flex flex-row text-white justify-center items-center bg-color border-b border-gray-900"}>
                  <div style={{width: 150}}>
                      <Link to={"/"} className={"flex justify-start items-center"}>
                          ...
                          <span className={"text-2xl font-extrabold sm:hidden xs:hidden"}>ICEBEAR</span>
                      </Link>
                  </div>
                  ...
                  <div className={"xs:flex-1 sm:flex-1 md:flex-1 flex justify-end items-center"} style={{width: 140}}>
                      {
                          wallet.isSignedIn() ?
                            ...
                            <Button onClick={() => login()} className={"flex justify-center items-center ant-button-hover-white"} shape="round">
              ...
              <Footer className={"flex flex-row justify-center items-center bg-color border-t border-gray-900 p-0"} style={{padding: 0}}>
                  <a href="#" className={"text-gray-500 p-5 hover:text-greenColor"}>
                      <Discord />
                  </a>
                  <a href="#" className={"text-gray-500 p-5 hover:text-greenColor"}>
                      <Telegram />
