...
const FaucetPage = () => {
    ...
    const handleMaxButton = () => {
        let currentShared = parseTokenWithDecimals(accountBalanceShare, getTokenMetadata("ICB").decimals);
        let maxSharePerAccount = parseTokenWithDecimals(faucetInfo.maxSharePerAccount, getTokenMetadata("ICB").decimals)
    ...
    const isDisable = () => {
        let currentShared = parseTokenWithDecimals(accountBalanceShare, getTokenMetadata("ICB").decimals);
        let maxSharePerAccount = parseTokenWithDecimals(faucetInfo.maxSharePerAccount, getTokenMetadata("ICB").decimals);
        let totalBalance = parseTokenWithDecimals(faucetInfo.totalBalanceShare, getTokenMetadata("ICB").decimals);
    ...
    const handleFaucet = async () => {
        ...
        try {
            // @ts-ignore
            await faucet(parseTokenAmount(faucetValue, getTokenMetadata("ICB").decimals).toLocaleString('fullwide', {useGrouping:false}));
        } catch (e) {
            console.log("Error", e);
        } finally {
          ...
          <section className="w-full md:w-560px lg:w-560px xl:w-560px m-auto relative xs:px-2">
              ...
              <div className={"flex flex-col mt-5 justify-between"}>
                  <div className={"flex flex-row justify-between mb-2 bg-cardBg rounded-2xl p-5 w-full"}>
                      ...
                      <p className={"text-2xl text-white"}>{formatNumber(parseTokenWithDecimals(faucetInfo.totalBalanceShare, getTokenMetadata("ICB").decimals))} ICB</p>
                  </div>
              </div>
              <div className={"bg-cardBg rounded-2xl p-5 mb-2"}>
                  <div className={"input-form mt-5"}>
                      <p className="flex flex-row items-center text-primaryText mb-2">
                          <CreditCardOutlined />
                          <span className="text-primaryText mr-2 ml-1">Your ICB shared balance:</span>
                          {formatNumber(parseTokenWithDecimals(accountBalanceShare, getTokenMetadata("ICB").decimals))}
                          <img className="mr-1 ml-2" style={{width: 15, height: 15}} src={getTokenMetadata("ICB").icon} alt=""/><span className="text-primary">ICB</span>
                      </p>
                      ...
                      <p className="text-xs text-primaryText mb-1">
                          {
                              accountBalanceShare == faucetInfo.maxSharePerAccount ?
                                <span>One account can get max {formatNumber(parseTokenWithDecimals(faucetInfo.maxSharePerAccount, getTokenMetadata("ICB").decimals))} ICB. You can not get more!</span>
                                :<span>One account can get max {formatNumber(parseTokenWithDecimals(faucetInfo.maxSharePerAccount, getTokenMetadata("ICB").decimals))} ICB. You can get more!</span>
                          }
                      </p>
              ...
              <div className="w-full grid mt-3 grid-cols-2 lg:grid-rows-2 gap-2">
                  ...
                  <div
                    className="lg:h-16 xs:h-20 md:h-20 rounded-lg bg-darkGradientBg shadow-dark p-2.5 hover:bg-darkGradientHoverBg">
                      ...
                      <div className="lg:flex lg:justify-center lg:items-center">
                          <label className="text-base font-medium text-xREFColor">{formatNumber(parseTokenWithDecimals(faucetInfo.totalShared, getTokenMetadata("ICB").decimals))}</label>
                          <label className="text-xs ml-1.5 text-primaryText">ICB</label>
                      </div>
                  </div>
              </div>
          </section>
